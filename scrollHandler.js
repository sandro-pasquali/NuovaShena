...
function scrollHandler(id,ref,t,w,h,spd,dist)
  {
	...
	this.scrollerHeight = (this.NS4) ? this.ref.clip.height
	                      : (this.MAC || this.NS6) ? this.ref.offsetHeight
    ...
    if(this.NS4)
	  {
        ...
        r.clip.top = this.scrollClipTop;
		r.clip.height = this.height;
	  }
	else 
	  {
		r.style.top = this.scrollTop;
        r.style.clip = 'rect(' + this.scrollClipTop + 'px ' + this.width + 'px ' + (this.height+this.scrollPosition) + 'px 0px)';
	  }    
	return;
  }  
